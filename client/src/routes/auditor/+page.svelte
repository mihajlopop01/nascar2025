        ...
        try {
            ...
            if (response.ok) {
                ...
                if (user.role === 'AUDITOR') {
                    isAuthorized = true; // Set authorized to true if the user is an auditor
                } else {
                    console.log('Access denied, redirecting to login'); // Log if access is denied
                    window.location.href = '/'; // Redirect to login if not an auditor
                }
            } else {
...
{:else if isAuthorized}
    <h1>Welcome to Auditor Dashboard</h1>
    <button on:click={handleLogout}>Logout</button>
    <!-- Auditor dashboard content goes here -->
{:else}
